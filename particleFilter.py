...
class PFLocaliser(PFLocaliserBase):
    ...
    def initialise_particle_cloud(self, initialpose):
        ...
        randomYaws = self.rndGen.normal(loc=meanOrientation, scale=stdOrientation, size=self.SIZE)
        rotatedQuaternions = [
        ...
        poseArray.poses = [
            Pose(
                position=Point(
                    x=float(rndPosition[0] + position.x * self.NOISE_AMPLIFICATION_CARTESIAN), 
                    y=float(rndPosition[1] + position.y * self.NOISE_AMPLIFICATION_CARTESIAN), 
                    z=0.0
                ),
                orientation=rndOrientation
            ) 
            for rndPosition, rndOrientation in zip(randomCartesianNoise, rotatedQuaternions)
        ]                                                                                                                                                                                                                         
        return poseArray
    ...
    def injectRandomParticles(self):
        ...
        if len(self.slow_ema) < self.slow_ema_param:
            self.slow_ema.append(averageWeight)   
            self._logger.info(f"Adding element to slow ema")
        
        if len(self.fast_ema) < self.fast_ema_param:
            self.fast_ema.append(averageWeight)     
            self._logger.info(f"Adding element to fast ema")
            
        num_random = int(self.SIZE * self.P_RANDOM)
        self._logger.info(f"number of random particles to be injected: {num_random}. Probability of random particles: {self.P_RANDOM}")
        
        if len(self.slow_ema) >= self.slow_ema_param and len(self.fast_ema) >= self.fast_ema_param:
            self._logger.info(f"INJECTING RANDOM PARTICLES")
            ...
            rotatedQuaternions = [rotateQuaternion(baseQuaternion, randomYaw) for randomYaw in randomYaws]
            
            self.particlecloud.poses.extend([
                Pose(
                    position=Point(x=self.rndGen.uniform(0, mapWidth), y=self.rndGen.uniform(0, mapHeight)),
    ...
    def update_particle_cloud(self, scan):
        ...
        for particle, tuple_val in self.particleWeights.items():
            probability = tuple_val[0] / self.totalWeight
            while pointer < cumulativeSum + probability:
                new_p = Pose()
                new_p.position.x = tuple_val[1].position.x
                new_p.position.y = tuple_val[1].position.y
                new_p.position.z = tuple_val[1].position.z
                new_p.orientation.x = tuple_val[1].orientation.x
                new_p.orientation.y = tuple_val[1].orientation.y
                new_p.orientation.z = tuple_val[1].orientation.z
                new_p.orientation.w = tuple_val[1].orientation.w
    ...
    def euclideanNorm(self, point1, point2): # point1 and point2 are vectors 
        # return np.sqrt(np.power(np.array([point1.position.x,point1.position.y])-np.array([point2.position.x, point2.position.y]),2).sum())
    ...
    def KmeansPlusPlus(self, particleCloud, intialCentroid, K): # K = number of clusters, data
        ...
        while len(centroids) < K:
            ...
            for point in particleCloud.poses:
                ...
                heapq.heappush(distancesMaxHeap, (-(minDistance * weight + diffYaw * self.diffYawInfluence), weight + self.rndGen.uniform(0, 0.1), point))
        ...
        while True:
            ...
            if len(combinationsPoints) > 1:
                for centroid1, centroid2 in combinationsPoints:
                    dist_metric = self.euclideanNorm(centroid1, centroid2) + self.diffYaw(centroid1, centroid2) * self.diffYawInfluence
                    
                    if dist_metric < self.mergeThreshold:
                        self._logger.info(f"Merging centroids! Distance {dist_metric:.2f} < Threshold {self.mergeThreshold}")
        ...
        self._logger.info(f"--- KmeansPlusPlus Finished. Returning {len(centroids)} centroids ---")
    ...
    def weightedAverageMean(self, particleCloud):
        ...
        # weights = [self.get_weight(particle) for particle in particleCloud]
        coords = [[particle.position.x, particle.position.y, particle.orientation.x, particle.orientation.y, particle.orientation.z, particle.orientation.w] for particle in particleCloud]
        averages = np.average(coords, weights=weights, axis=0)
        q_avg = averages[2:] 
        v_mag = np.linalg.norm(q_avg)
        if v_mag > 0:
            q_avg /= v_mag
        new_pose = Pose(position=Point(x=averages[0], y=averages[1]), orientation=Quaternion(x=q_avg[0], y=q_avg[1], z=q_avg[2], w=q_avg[3]))
    ...
    #TODO: Implement ISODATA inspired version of K-means++ by starting with high number of clusters and iterating down to achieve an adaptive clustering algorithm. (reference: http://www.wu.ece.ufl.edu/books/EE/communications/UnsupervisedClassification.html)
    ...
    def estimate_pose(self):
        ...
        :Return:
         ...
         """
        # Implement K-means++ and weighted average for pose estimation. If distance between centroids with the 
        # K-means clusters is less than a threshold, revert to weighted average. Use highest weighted particle as initial centroid.
        centroids = self.KmeansPlusPlus(self.particlecloud, self.maxWeightParticle, K=self.NUM_CLUSTERS)
        combinationsPoints = list(combinations(centroids, 2))
   
        maxDistance = np.max([self.euclideanNorm(point1, point2) for point1, point2 in combinationsPoints])
      
        # pose = centroids[np.argmax([self.particleWeights[str(particle)][0] for particle in centroids])]
        pose = centroids[np.argmax([self.get_weight(particle)  for particle in centroids])]
        return pose
